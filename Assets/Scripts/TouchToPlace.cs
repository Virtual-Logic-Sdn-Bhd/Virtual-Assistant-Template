    ...
    private void FingerDown(EnhancedTouch.Finger finger)
    {
            ...
            hits, TrackableType.PlaneWithinPolygon))
        {
            foreach(ARRaycastHit hit in hits)
            {
                ...
                if (aRPlaneManager.GetPlane(hit.trackableId).alignment == 
                    PlaneAlignment.HorizontalUp)
                {
                    ...
                    Vector3.Scale(targetRotationEuler, obj.transform.up.normalized);
                    Vector3 scaledEuler = transform.eulerAngles * Mathf.Deg2Rad;
